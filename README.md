...
1. `./lab.sh seed` genera datos sucios en `data/raw/`, simulando la ingesta inicial.
...
6. `./lab.sh train` entrena los modelos ML disponibles segun los flags del laboratorio.
...
Cada ejercicio consume datasets preparados en `data/gold/` por el ETL. El estudiante entrena modelos, guarda artefactos en `data/gold/models/` y expone metricas para que la web pueda mostrar si la solucion funciona.
...
1. El comando `./lab.sh train` termina sin errores.
