...
def import_block(package: str, name: str, unlocked: bool) -> ModuleType:
    """Devuelve el módulo del bloque: solución si procede, scaffold si no.

    Un bloque vive en dos ficheros: `<name>.py` (solución) y `<name>_ex.py`
    (scaffold que completa el alumno). El flag LAB_* decide cuál se sirve.

    La solución solo se importa si está desbloqueada Y presente. En la
    distribución que reciben los alumnos los ficheros solución no se
    incluyen, así que ahí el flag no puede destapar nada: si falta el
    módulo, se sirve el scaffold en vez de romper el arranque de la app.

    Args:
        package: paquete contenedor (p. ej. "src.web.routes").
        name: nombre base del bloque, sin sufijo (p. ej. "neo4j_basic").
        unlocked: si el flag marca este bloque como desbloqueado.
    """
    if unlocked:
        try:
            return importlib.import_module(f"{package}.{name}")
        except ModuleNotFoundError:
            # Distribución sin soluciones: se degrada al scaffold.
            pass
    return importlib.import_module(f"{package}.{name}_ex")
