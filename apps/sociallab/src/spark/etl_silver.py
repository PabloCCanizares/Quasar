...
def _raw_quality_profile(spark: SparkSession, raw_path: str, silver_path: str) -> dict:
    """Radiografía de calidad de la capa RAW (donde vive la suciedad).

    A diferencia de PreproLab/LLM Lab, el silver de SocialLab ya está limpio,
    así que la suciedad interesante está en raw. Perfilamos ahí: filas y nulls
    por columna de las 4 tablas crudas. Se escribe en `silver/_profile.json`,
    misma ubicación que el resto de apps para que el Hub lo sirva igual.
    """
    profile: dict = {}
    for table in ("users", "posts", "likes", "follows"):
        try:
            df = spark.read.json(f"{raw_path}/{table}.json")
        except Exception:  # noqa: BLE001 — tabla ausente, se omite del perfil
            continue
        total = df.count()
        ...
        profile[table] = {"rows": total, "columns": columns}

    silver = Path(silver_path)
    silver.mkdir(parents=True, exist_ok=True)
    (silver / "_profile.json").write_text(
        json.dumps(profile, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print(f"[silver] radiografía de calidad (raw) → {silver / '_profile.json'}")
    return profile
...
def run_silver(spark: SparkSession, raw_path: str, silver_path: str):
    ...
    # Radiografía de calidad del raw (para el Hub: "tus datos ahora mismo").
    _raw_quality_profile(spark, raw_path, silver_path)
