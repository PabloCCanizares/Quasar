...
def run_model(name: str) -> dict:
    ...
    # La solución solo se usa si el bloque está desbloqueado Y el módulo
    # existe: en la distribución que reciben los alumnos no se incluye, y
    # entonces se ejecuta el scaffold en vez de romper con ImportError.
    module = None
    if block in unlocked:
        try:
            module = importlib.import_module(f"src.spark.models.{name}")
        except ModuleNotFoundError:
            module = None
    if module is None:
        module = importlib.import_module(f"src.spark.models_ex.{name}")
