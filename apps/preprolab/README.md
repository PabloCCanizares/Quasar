...
## Estado actual: COMPLETA (8 bloques + Pipeline Studio — 37 ejercicios)

Dataset generado y los 8 bloques del Tema 5 funcionales, más el Pipeline Studio. Ingesta a la capa silver con **Spark ETL** (`./lab.sh preprolab etl`); los bloques corren sobre esa capa en pandas/sklearn.

| Bloque | Técnicas | Ejercicios |
|---|---|---|
| **Seed** | Generador de la flota de robots (4 tablas, 14 problemas inyectados) | — |
| **Spark ETL** | raw JSONL → silver parquet (columnar) + perfil de calidad; suciedad preservada | — |
| `eda` | Univariable, missing matrix, correlaciones — UI con Plotly | 3 |
| `missing` | Drop + simple (mean/median/mode) + KNN + KMeans + comparativa antes/después | 5 |
| `outliers` | IQR + Z-score + gestión (remove/cap/log) + noise filters EF/CVCF/IPF | 4 |
| `integration` | union + 4 joins + Pearson + Cramér's V + dedup por correlación | 4 |
| `transform` | One-hot + ordinal + multi-flag CSV + discretización (eq-width/eq-freq/MDLP) + groupby | 5 |
| `normalize` | Z-score + Min-Max + Robust + Decimal Scaling + comparativa con detección de outliers | 5 |
| `reduce_dim` | PCA + t-SNE + feature selection Filter / Wrapper / Embedded + comparador de las 3 familias | 6 |
| `reduce_inst` | SRSWOR + estratificado + balanceado + por clusters + K-Means compresión | 5 |
| **★ Pipeline Studio** | Compose pipelines + RF training + comparativa AUC/F1/ROC entre N configuraciones | — |
...
**Total**: 37 ejercicios con patrón scaffold/solución (conteo autoritativo en el catálogo del Hub).
...
./lab.sh preprolab seed      # Genera la flota de robots (raw JSONL)
./lab.sh preprolab etl       # Spark ETL: raw → silver parquet + perfil de calidad
...
│   ├── seed/        # Generador del dataset sintético
│   └── spark/       # ETL Spark: raw JSONL → silver parquet + perfil de calidad
├── main.py
├── Dockerfile
├── requirements.txt
└── .env.example
...
Cada bloque (`eda`, `missing`, `outliers`, `integration`, `transform`, `normalize`, `reduce_dim`, `reduce_inst`) expone sus endpoints bajo `/api/preprolab/<bloque>/…` (detallados en las secciones de arriba). Los endpoints leen la capa silver (parquet) generada por el ETL, con fallback a raw JSON.
